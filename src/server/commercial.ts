...
export async function listCommercialOverview(
  ...
  const [sources, baselineRows, items] = await Promise.all([
    getDb()
      .select({
    ...
    getDb()
      ...
      .orderBy(desc(commercialBaselineVersions.versionNumber))
      .limit(1),
    getDb()
      .select({
      ...
      .where(eq(commercialScopeItems.projectId, projectId))
      .orderBy(
...
export async function createCommercialScopeItem(
  ...
  const itemId = await getDb().transaction(async (transaction) => {
    ...
    await assertScopeCapacity(transaction, projectId);
    const sourceId = await getBaselineVersionSource(
      transaction,
      projectId,
...
export async function listCommercialDrift(
  ...
  return {
    data: rows.map((row) => ({
      ...row,
      state: commercialState(row.purpose, row.basisCount),
    })),
    page: {
      number: filters.page,
...
async function getBaselineVersionSource(
  transaction: Transaction,
  projectId: string,
  baselineVersionId: string,
) {
  const rows = await transaction
    .select({ sourceId: commercialBaselineVersions.sourceId })
    .from(commercialBaselineVersions)
    .where(
      and(
        eq(commercialBaselineVersions.id, baselineVersionId),
...
async function insertAnchors(
  ...
  if (anchors.some((anchor) => anchor.sourceId !== expectedSourceId)) {
    throw conflict(
      "evidence_source_mismatch",
      "Scope evidence must come from this baseline version's source.",
    );
  }
  const source = await transaction
    .select({ text: commercialEvidenceSources.extractedText })
    .from(commercialEvidenceSources)
    .where(
      and(
        eq(commercialEvidenceSources.id, expectedSourceId),
        eq(commercialEvidenceSources.projectId, projectId),
        eq(commercialEvidenceSources.parseState, "ready"),
      ),
    )
    .limit(1);
  if (!source[0]?.text) throw notFound();
  if (
    anchors.some(
      (anchor) =>
        anchor.endOffset <= anchor.startOffset ||
        anchor.endOffset > source[0]!.text!.length,
    )
  ) {
    throw conflict(
